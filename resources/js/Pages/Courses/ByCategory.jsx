...
export default function Index({ auth, courses, filters, filterOptions, category }) {
    ...
    const content = currentCategory 
        ? categoryContent[currentCategory.slug] || null
        : null;
    ...
    const getActiveFilterCount = () => {
        return Object.entries(selectedFilters).filter(([key, value]) => 
            ...
            key !== 'sort_field' && key !== 'sort_direction' && key !== 'page'
        ).length;
    };

    return (
        <GuestLayout auth={auth}>
            <Head title={currentCategory ? `${content?.title || currentCategory.name} | IGRCFP` : 'All Courses | IGRCFP'} />
                    ...
                    <div className="bg-gradient-to-r from-blue-900 to-blue-800 rounded-2xl p-8 md:p-12">
                        <div className="text-center">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                <Link href={route('courses.index')} className="bg-white text-blue-900 px-8 py-3 rounded-lg font-semibold hover:bg-blue-50 transition duration-300">
                                    Explore Certifications
                                </Link>
                                <Link href={route('contact')} className="bg-transparent border-2 border-white text-white px-8 py-3 rounded-lg font-semibold hover:bg-white hover:text-blue-900 transition duration-300">
                                    Contact Admissions
                                </Link>
                            </div>
                        </div>
