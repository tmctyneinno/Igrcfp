...
export default function EnrollmentIndex({ course, enrollment, modules: initialModules = [], candidate, examResults }) {
    ...
    const handleIdentityVerification = async () => {
        setIsVerifying(true);
        try {
            // Simulate camera capture - In production, use actual camera API
            const response = await router.post(route('exam.verify-identity', enrollment.id), {
                image: capturedImage
            }, {
                preserveScroll: true,
                onSuccess: () => {
                                ...
                                {!enrollment?.identity_verified ? (
                                    ...
                                    <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-green-600">Verified on {new Date(enrollment?.verified_at).toLocaleDateString()}</p>
                                        </div>
                                    </div>
                                )}
                            </motion.div>
