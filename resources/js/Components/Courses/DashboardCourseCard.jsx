...
export default function DashboardCourseCard({ course, onAddToCart, onScholarshipEnroll, isInCart, isAdding }) { 
    ...
    const handleAddToCartClick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        if (onAddToCart && course) onAddToCart(course);
    };

    const handleScholarshipClick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        if (onScholarshipEnroll && course) onScholarshipEnroll(course);
    };

    const handleViewCartClick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        if (typeof route === 'function') window.location.href = route('dashboard.cart.index');
    };

    return (
        <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition transform hover:-translate-y-2 h-full flex flex-col" data-aos="fade-up"> 
            <Link href={route('dashboard.courses.show', course?.slug)} className="block h-48 overflow-hidden">
                <img src={course?.image_url || '/images/fallback-course.jpg'} alt={course?.title || 'Course image'} className="w-full h-full object-cover transition-transform duration-300 hover:scale-110 cursor-zoom-in" onError={(e) => { e.target.src = '/images/fallback-course.jpg'; }} />
            </Link>

            <div className="p-2 flex-1 flex flex-col">
                <Link href={route('dashboard.courses.show', course?.slug)}>
                    <h4 className="text-lg font-semibold text-gray-900 mb-2 hover:text-blue-900 transition line-clamp-2">{course?.title || 'Untitled Course'}</h4>
                </Link> 
                
                <p className="text-gray-600 text-sm mb-1 line-clamp-2 flex-1">{getCleanDescription()}</p>

                <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-semibold px-3 capitalize py-1 bg-blue-100 text-blue-800 rounded-full">{course?.level || 'All Levels'}</span>
                    <div className="flex items-center space-x-3">
                        {course?.duration && <span className="text-sm text-gray-500">⏱️ {course.duration}</span>}
                        {course?.modules_count > 0 && <span className="text-sm text-gray-500">📚 {course.modules_count} modules</span>}
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-1 border-t">
                    ...
                    {course?.is_enrolled ? (
                        <Link href={route('dashboard.courses.show', course.slug)} className="inline-flex items-center px-2 py-1.5 bg-green-600 text-white text-sm font-semibold rounded-lg hover:bg-green-700 transition transform hover:-translate-y-1">Continue Course</Link>
                    ) : canUseScholarship ? (
                        /* SCHOLARSHIP BUTTON */
                        <button
                        ...
                        <div className="flex gap-2">
                            <Link href={route('dashboard.cart.index')} onClick={handleViewCartClick} className="inline-flex items-center px-3 py-1.5 bg-green-600 text-white text-sm font-semibold rounded-lg hover:bg-green-700 transition transform hover:-translate-y-1">View Cart</Link>
                        </div>
                    ) : (
                        <button onClick={handleAddToCartClick} disabled={isAdding} className="inline-flex items-center px-2.5 py-1.5 bg-blue-900 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition transform hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed">
                            {isAdding ? (
                                <>
                                ...
                                <> Add to Cart<svg className="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg></>
                            )}
                        </button>
                    )}
                </div>
